...
def retrieve_element_value(element, name):
    ...
    if retrieved_element is None:
        ...
        logger.debug("retrieve_element_value: %s (%s) is %s" % (name, retrieved_element.tag, retrieved_element.text))
...
def parse_reporting_org(element):
    ...
    logger.debug("parse_reporting_org: %s, %s, %s" % (ref, type, organisation_name,))
    # Do stuff with model
    # Return model
...
def parse_other_identifier(element):
    ...
    logger.debug("parse_other_identifier: %s, %s, %s" % (owner_name, owner_ref, other_identifier))
...
def parse_description(element):
    ...
    logger.debug("parse_description: %s (type: %s)" % (description, type))
...
def parse_activity_date(element):
    ...
    logger.debug("parse_activity_date: %s (%s)" % (iso_date, activity_date_type))
...
def parse_contact_info(element):
    ...
    logger.debug("parse_contact_info: %s - %s - %s" % (job_title, telephone, email))
...
def parse_xml(xml_file):
    for _, element in etree.iterparse(xml_file, tag="iati-activity", remove_comments=True):
        print "Iati activity"
...
def parse_activity(activity_element):
    ...
    generated_datetime = activity_element.get("generated-datetime", default=None)
    default_currency = activity_element.get("default-currency", default=None)
    ...
    logger.debug("parse_activity: attribute generated-datetime: %s" % (generated_datetime))
    logger.debug("parse_activity: attribute default-currency: %s" % (default_currency))
    logger.debug("parse_activity: attribute hierarchy: %s" % (hierarchy))
    logger.debug("parse_activity: attribute linked-data-uri: %s" % (linked_data_uri))
    

    
    for element in activity_element:
        result = parser_105_dictionary.get(element.tag, parse_undefined_element)(element) 
